...
<html>
    ...
    <body>
      ...
      long staff_id = Integer.parseInt(id);
       
      Class.forName("oracle.jdbc.driver.OracleDriver");
      Connection con = DriverManager.getConnection("jdbc:oracle:thin:@localhost:1521:xe","vstar","vaibhav");
      Statement stmt = con.createStatement(); 
      ResultSet rs = stmt.executeQuery("select STAFF_ID,FNAME,MNAME,LNAME,To_char(DOB,'dd-mm-yyyy')"
              + ",ADHAR_NO,PAN_NO,MOB_NO,EMAIL_ID,C_ADDRESS,P_ADDRESS,JOB_PROFILE,EDUCATION,To_char(HIREDATE,'dd-mm-yy')"
              + ",SALARY,EXPRIENCE,USERNAME,PASSWORD from chitboy WHERE  STAFF_ID =  "+staff_id+" ");
       
     if (rs.next())
     {  %>  
  <div class="container">
        <h1>Edite Information Of Up Krushi Adhikari  </h1>
        <form action="finalupdatechitboy.jsp" method="POST">

            <div class="form-row">
                <div class="form-group">
                    <label for="staff_id">Staff_Id :</label>
                    <input type="number" id="Staff_Id" name="StaffId" value="<%=rs.getLong(1)%>"  readonly>
                </div>
